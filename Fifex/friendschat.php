    ...
    if (isset($_POST['submit']))
    {
        try
        {
            if ($_POST['text'] != null)
            {
                ...
                $sql = "INSERT INTO user_message_table (user_1, user_2, message_text) VALUES (?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("sss", $s_user_nick, $_SESSION['nick'], $message_text);
                $stmt->execute();
            }
        }
        catch (Exception $e)
...
<body>
    ...
    <script>
        function fetchMessages() { $.ajax({ url: 'user_fetch_messages.php', method: 'POST', success: function(data) { $('#message-container').html(data); } }); }
